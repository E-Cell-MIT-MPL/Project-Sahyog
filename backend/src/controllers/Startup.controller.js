...
const RegisterStartup = asyncHandler(async (req, res) => {
  try {
    ...
    if (
      [Startupname, password, email, phoneNumber].some(
        (field) => !field?.trim(),
      )
    ) {
...
const Startuplogin = asyncHandler(async (req, res, next) => {
  ...
  const { accesstoken, refreshtoken } =
    await generateaccesstokenandrefreshtoken(Startup._id);
  ...
  return res
    ...
    .json({
      message: "User logged in sucsessfully",
      accesstoken,
      refreshtoken,
      Startup,
    });
...
const changestartuppasswordatlogin = asyncHandler(async (req, res, next) => {
  try {
    ...
    return res
      .status(200)
      .json({ message: "Password changed successfully for logged-in user" });
  } catch (error) {
    console.log(error);
  }
});
